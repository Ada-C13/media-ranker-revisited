...
describe WorksController do
  ...
  describe "Member user" do 
    ...
    describe "upvote" do
      it "redirects to the root page if no user is logged in" do
        ...
        expect{post upvote_path(@work1.id)}.wont_change "Vote.count"
        must_redirect_to root_path
      end

      it "succeeds for a logged-in user and a fresh user-vote pair" do
      
        expect{post upvote_path(@work2.id)}.must_change "Vote.count", 1
  ...
  describe "GUEST USER" do
    ...
    it "cannot update work" do
      expect {
        put work_path(@work1.id), params: @new_work
      }.wont_change "Work.count"
      must_redirect_to root_path
      expect(flash[:result_text]).must_equal "You must be logged in to do that"
    end
    ...
    it "cannot destroy work" do
      expect {
        delete work_path(@work1.id)
      }.wont_change "Work.count"
      must_redirect_to root_path
      expect(flash[:result_text]).must_equal "You must be logged in to do that"
    end

    it "cannot vote for work" do
      expect {
